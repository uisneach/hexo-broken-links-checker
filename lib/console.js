        ...
        if (config.broken_links_checker) {
            ...
            if (enable) {
                return hexo.call('generate', {}).then(async function () {
                    ...
                    // Get external links from pages
                    const pages = hexo.locals
                        .get('pages')
                        .data.map(({ content, path, ...page }) => {
                            return {
                                path,
                                externalLinks: [
                                    ...new Set(
                    ...
                    const allExternalLinks = [...posts, ...pages];


                    const externalLinks = allExternalLinks.reduce((result, current) => {
